...
public class DBConnection { // класс для подключение к DB
    private static Connection connection; //интерфейс Connection позляет подключится к DB
    private static Statement statement; //интерфейс Statement позляет запустить запрос и возвратит результат
    private static QueryRunner queryRunner; //интерфейс QueryRunner будет запустит запросы

    private DBConnection() {
    } //private конструктор не позволяет создать объект этого класса
